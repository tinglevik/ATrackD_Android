...
import com.example.atrackd.data.AppDatabase
...
class ATrackDApplication : Application() {
        ...
        applicationScope.launch {
            repository.getQuickPanelActivities().collect { activities ->
                if (activities.isNotEmpty()) {
                    val intent = Intent(this@ATrackDApplication, ActivityTrackerService::class.java)
                    try {
                        ContextCompat.startForegroundService(this@ATrackDApplication, intent)
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
